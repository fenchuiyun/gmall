...
//                "  }\n" +
//                "}";
        //map.put("doc",new User("xujiqiang",null,null));
//        Index index = new Index.Builder(new User("bingbing","xjq",22)).index("user").type("info").id("1").build();
        //Update index = new Update.Builder(map).index("user").type("info").id("1").build();
//        jestClient.execute(index);
        //Update update = new Update.Builder(new User("fenchuiyun","qwe",1)).index("user").type("info").id("1").build();
        ...
        do {
            ...
            for (SpuInfoEntity spuInfoEntity : spuInfoEntities) {
                ...
                skuInfoEntities.forEach(skuInfoEntity -> {
                    ...
                    // 设置搜索属性
                    Resp<List<SpuAttributeValueVO>> searchAttrValueResp = this.gmallPmsClient.querySearch(spuInfoEntity.getId());
